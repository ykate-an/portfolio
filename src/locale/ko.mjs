const ko = {
  /* header */
  HEADER_NAME: "안여경",
  HEADER_JOB_POSITION: "Web Developer",
  HEADER_EMAIL: "yeokyeong1004@gmail.com",
  HEADER_PHONE: "010 8632 0833",
  HEADER_GITHUB: "github.com/yeokyeong",
  HEADER_RESUME_LINK: "yeokyeong-resume.github.io",

  /* intro */
  INTRO_TITLE: "Hi. I'm YeoKyeong An,",
  INTRO_HINT: "Tip: you can just call me Kate😄",

  INTRO_DESCRIPTION:
    "안녕하세요 웹 개발(프론트엔트/백엔드) 2년차 개발자 안여경입니다.",

  /* utils */
  UTILS_DOWNLOAD_TITLE: "이력서",

  /* education */
  EDUCATION_UNIVERSITY_NAME: "가톨릭대학교",
  EDUCATION_UNIVERSITY_MAJOR: "미디어기술콘텐츠학과 - 미디어공학전공",
  EDUCATION_UNIVERSITY_PERIOD: "2015.03 - 2017.08",
  EDUCATION_UNIVERSITY_SCORE: "3.8 /4.5",

  /* license/awards */
  LICENSE_ENGINEER_NAME: "정보처리기사",
  LICENSE_ENGINEER_PART: "한국산업인력공단",
  LICENSE_ENGINEER_TIME: "2016",
  AWARD_CJ_NAME: "CJ 문화산업 전략 PT 경진대회",
  AWARD_CJ_PART: "CJ그룹",
  AWARD_CJ_TIME: "2015",

  /* languages */
  LANGUAGE_TYPE: "English",
  LANGUAGE_LEVEL: "Beginner",

  /* interests */
  INTEREST_READING: "책읽기",
  INTEREST_COOKING: "요리",
  INTEREST_SURFING: "서핑",
  INTEREST_SWIMMING: "수영",

  /* skills */
  SKILL_NODE: "Node.js",
  SKILL_DOCKER: "Docker",
  SKILL_NGINX: "Nginx",
  SKILL_PHP: "PHP",
  SKILL_REACT: "React",
  SKILL_HTML_CSS: "HTML/CSS",
  SKILL_JAVASCRIPT: "JavaScript",
  SKILL_FLUTTER: "Flutter",
  SKILL_BOOTSTRAP: "Bootstrap",
  SKILL_AWS: "Aws",
  SKILL_ZENKINS: "Zenkins",
  SKILL_GIT: "Git",
  SKILL_NOTION: "Notion",

  /* work experiences */

  WORK_SDSE_NAME: "Samsung SDSE",
  WORK_SDSE_POSITION: "Junior Web Developer",
  WORK_SDSE_PERIOD: "2019.08 - 2021.03",
  WORK_SDSE_DESC:
    "Enim sint amet Lorem occaecat velit proident id culpa cupidatat deserunt.",

  WORK_CB_NAME: "Codebrick",
  WORK_CB_POSITION: "Junior Web Developer",
  WORK_CB_PERIOD: "2019.08 - 2021.03",
  WORK_CB_DESC:
    "Enim sint amet Lorem occaecat velit proident id culpa cupidatat deserunt.",

  WORK_WIZ_NAME: "위즈스쿨",
  WORK_WIZ_POSITION: "Junior Web Developer",
  WORK_WIZ_PERIOD: "2019.08 - 2021.03",
  WORK_WIZ_DESC:
    "코딩교육 회사로 회사 서비스의 웹 프론트엔드와 백엔드 업무를 담당",

  WORK_HACKERS_NAME: "모파스(해커스홀딩스)",
  WORK_HACKERS_POSITION: "Junior Web Developer",
  WORK_HACKERS_PERIOD: "2019.01 - 2019.07",
  WORK_HACKERS_DESC:
    "블록체인 관련 회사로 디앱 제작 및 웹 프론트엔드와 백엔드 업무를 담당",

  /* work details */
  WORK_PROJECT_ROLE_PARTICIPATION: "참여",
  WORK_PROJECT_ROLE_LEADER: "리더",
  WORK_PROJECT_DESC: "설명",
  WORK_PROJECT_ROLE: "역할",
  WORK_PROJECT_FEATURE: "기능",
  WORK_PROJECT_OUTCOME: "성과",
  WORK_PROJECT_PERIOD: "기간",
  WORK_PROJECT_DETAIL: "세부사항",
  WORK_PROJECT_APPENDIX: "첨부",

  /* Samsung SDSE */
  WORK_SDSE_PROJECT_01_TITLE: "SDSE project 1",
  WORK_SDSE_PROJECT_01_PERIOD: "2019.10 - 2020.01",
  WORK_SDSE_PROJECT_01_DESC:
    "Online video coding education platform by using webRTC",
  // WORK_SDSE_PROJECT_01_FEATURES:
  //   "온라인 수업, 선생님과 학생의 에디터 상태 공유, SMS, 수업예약 등",
  WORK_SDSE_PROJECT_01_OUTCOME: "Launch of `Wizlive` service",
  // WORK_SDSE_PROJECT_01_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_SDSE_PROJECT_01_ROLE:
    "Developed class reservation system and Developed at home page part of front-end,back-end",
  WORK_SDSE_PROJECT_01_DETAILS_01:
    " Developed reservation of teacher/student(class synchronization, SMS, db table structure, UI)",
  WORK_SDSE_PROJECT_01_DETAILS_02:
    "Developed student's class state on back-end and front-end",
  WORK_SDSE_PROJECT_01_DETAILS_03: "Developed main page UI of service",

  WORK_SDSE_PROJECT_02_TITLE: "SDSE project 2",
  WORK_SDSE_PROJECT_02_PERIOD: "2020.02 - 2020.04",
  WORK_SDSE_PROJECT_02_DESC: " Separated services user service from IDE editor",
  // WORK_SDSE_PROJECT_02_FEATURES:
  //   "코드 에디터, 에디터 튜토리얼 , 구독, 좋아요, 게시판 등",
  WORK_SDSE_PROJECT_02_OUTCOME:
    "Got more users, created user community and launched of service",
  // WORK_SDSE_PROJECT_02_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_SDSE_PROJECT_02_ROLE:
    "Constructured entire page layout and updated IDE UI",
  WORK_SDSE_PROJECT_02_DETAILS_01:
    "Developed new pages and reconstructured page because of seperated service",
  WORK_SDSE_PROJECT_02_DETAILS_02: "Reconstructured IDE ,updated of entire UI ",

  /* Codebrick */
  WORK_CB_PROJECT_01_TITLE: "codebrick project 1",
  WORK_CB_PROJECT_01_PERIOD: "2019.10 - 2020.01",
  WORK_CB_PROJECT_01_DESC:
    "Online video coding education platform by using webRTC",
  // WORK_CB_PROJECT_01_FEATURES:
  //   "온라인 수업, 선생님과 학생의 에디터 상태 공유, SMS, 수업예약 등",
  WORK_CB_PROJECT_01_OUTCOME: "Launch of `Wizlive` service",
  // WORK_CB_PROJECT_01_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_CB_PROJECT_01_ROLE:
    "Developed class reservation system and Developed at home page part of front-end,back-end",
  WORK_CB_PROJECT_01_DETAILS_01:
    " Developed reservation of teacher/student(class synchronization, SMS, db table structure, UI)",
  WORK_CB_PROJECT_01_DETAILS_02:
    "Developed student's class state on back-end and front-end",
  WORK_CB_PROJECT_01_DETAILS_03: "Developed main page UI of service",

  WORK_CB_PROJECT_02_TITLE: "codebrick project 2",
  WORK_CB_PROJECT_02_PERIOD: "2020.02 - 2020.04",
  WORK_CB_PROJECT_02_DESC: " Separated services user service from IDE editor",
  // WORK_CB_PROJECT_02_FEATURES:
  //   "코드 에디터, 에디터 튜토리얼 , 구독, 좋아요, 게시판 등",
  WORK_CB_PROJECT_02_OUTCOME:
    "Got more users, created user community and launched of service",
  // WORK_CB_PROJECT_02_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_CB_PROJECT_02_ROLE:
    "Constructured entire page layout and updated IDE UI",
  WORK_CB_PROJECT_02_DETAILS_01:
    "Developed new pages and reconstructured page because of seperated service",
  WORK_CB_PROJECT_02_DETAILS_02: "Reconstructured IDE ,updated of entire UI ",

  /* wizschool */
  WORK_WIZ_PROJECT_01_TITLE: "서비스 ‘위즈라이브’ 개발",
  WORK_WIZ_PROJECT_01_PERIOD: "2019.10 - 2020.01",
  WORK_WIZ_PROJECT_01_DESC:
    "회사 온라인 IDE에 webRTC를 이용한 온라인 화상 코딩 교육 플랫폼",
  // WORK_WIZ_PROJECT_01_FEATURES:
  //   "온라인 수업, 선생님과 학생의 에디터 상태 공유, SMS, 수업예약 등",
  WORK_WIZ_PROJECT_01_OUTCOME: "서비스런칭",
  // WORK_WIZ_PROJECT_01_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_01_ROLE:
    "수업 예약 시스템 클라이언트/서버 개발 및 플랫폼 홈페이지 클라이언트/서버 개발",
  WORK_WIZ_PROJECT_01_DETAILS_01:
    "튜터/학생 예약 시스템 구현(수업 동기화, 문자전송, db 테이블 구조화, UI 개발)",
  WORK_WIZ_PROJECT_01_DETAILS_02: "학생 수업 현황 클라이언트/서버 개발",
  WORK_WIZ_PROJECT_01_DETAILS_03: "메인 페이지 개발",

  WORK_WIZ_PROJECT_02_TITLE: "서비스 ‘위즈랩’ 개발 및 보완",
  WORK_WIZ_PROJECT_02_PERIOD: "2020.02 - 2020.04",
  WORK_WIZ_PROJECT_02_DESC:
    "기존 서비스에서 온라인 IDE 기능과 사용자 관련된 서비스를 독립된 플랫폼으로 분리",
  // WORK_WIZ_PROJECT_02_FEATURES:
  //   "코드 에디터, 에디터 튜토리얼 , 구독, 좋아요, 게시판 등",
  WORK_WIZ_PROJECT_02_OUTCOME: "서비스런칭, 유저 유치 및 커뮤니티 생성",
  // WORK_WIZ_PROJECT_02_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_02_ROLE:
    "전체 페이지 레이아웃 구조화 및 온라인 IDE UI 업데이트 진행",
  WORK_WIZ_PROJECT_02_DETAILS_01:
    "서비스 분리에 따른 페이지 재구조화 및 새로운 페이지 개발",
  WORK_WIZ_PROJECT_02_DETAILS_02: "온라인 IDE 리팩토링, 전체 UI 업데이트",

  WORK_WIZ_PROJECT_03_TITLE: "회사 소개페이지 개발",
  WORK_WIZ_PROJECT_03_PERIOD: "2020.02(1주)",
  WORK_WIZ_PROJECT_03_DESC: "회사 소개 및 정보 안내 페이지 개발",
  // WORK_WIZ_PROJECT_03_FEATURES: "회사에 대한 소개 홈페이지",
  WORK_WIZ_PROJECT_03_OUTCOME: "회사 홈페이지",
  // WORK_WIZ_PROJECT_03_TECH: "react,js,HTML/CSS",
  WORK_WIZ_PROJECT_03_ROLE:
    "프로젝트에 혼자 참여하여 클라이언트 및 서버 개발 완료",
  WORK_WIZ_PROJECT_03_DETAILS_01: "UI 퍼블리싱",

  WORK_WIZ_PROJECT_04_TITLE: "중국 서비스 ‘자기주도학습’ 개발",
  WORK_WIZ_PROJECT_04_PERIOD: "2020.02 - 2020.03",
  WORK_WIZ_PROJECT_04_DESC:
    "학생이 코드를 작성해 정답을 맞춰 스스로 코딩 공부하는 학습 플랫폼",
  // WORK_WIZ_PROJECT_04_FEATURES:
  //   "학생이 코드를 작성해 정답을 맞춰가며 스스로 코딩에 대해 공부하는 학습 모듈",
  WORK_WIZ_PROJECT_04_OUTCOME: "서비스런칭",
  // WORK_WIZ_PROJECT_04_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_04_ROLE:
    "클라이언트에서 구현된 모듈에 맞춰 서버 개발 담당 및 모듈에 맞춰 UI 개발 참여",
  WORK_WIZ_PROJECT_04_DETAILS_01:
    "자기주도학습 모듈에 맞춰 db 설계 및 api 구현",
  WORK_WIZ_PROJECT_04_DETAILS_02: "자기주도학습 클라이언트 구현 및 ui 개발",

  WORK_WIZ_PROJECT_05_TITLE: "정부주최 ‘온라인코딩파티’ 서버/클라이언트 개발",
  WORK_WIZ_PROJECT_05_PERIOD: "2020.06",
  WORK_WIZ_PROJECT_05_DESC:
    "단계별 코딩 미션을 제공하여 학생들이 코딩을 쉽게 배울수 있도록 정부가 주최하는 이벤트",
  // WORK_WIZ_PROJECT_05_FEATURES: "단계별 미션,참여자 인증서발급",
  WORK_WIZ_PROJECT_05_OUTCOME: "최종 참여자 38,000명, 유저 유치 5,600명 ",
  // WORK_WIZ_PROJECT_05_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_05_ROLE:
    "프로젝트에 혼자 참여하여 클라이언트 및 서버 개발 완료",
  WORK_WIZ_PROJECT_05_DETAILS_01:
    "클라이언트 요구사항에 맞춰 미션 코드 검사 로직 추가 및 수정",
  WORK_WIZ_PROJECT_05_DETAILS_02:
    "참여 완료/단계/레벨 통과 등의 클라이언트/서버 구현",
  WORK_WIZ_PROJECT_05_DETAILS_03: "프로젝트 관련 이벤트 서버/클라이언트 개발",
  WORK_WIZ_PROJECT_05_DETAILS_04:
    "사용자 세션 유지, 사용자 기존데이터 연동 및 abusing 제한",

  WORK_WIZ_PROJECT_06_TITLE: "삼성 소프트웨어 아카데미 강의",
  WORK_WIZ_PROJECT_06_PERIOD: "2020.07(1일)",
  WORK_WIZ_PROJECT_06_DESC:
    "삼성&멀티캠퍼스에서 주최하는 교육 프로그램으로 1일 강사로 참여",
  // WORK_WIZ_PROJECT_06_FEATURES: "",
  WORK_WIZ_PROJECT_06_OUTCOME: "",
  // WORK_WIZ_PROJECT_06_TECH: "js",
  WORK_WIZ_PROJECT_06_ROLE: "하루동안 대학생 대상으로 강의를 진행",

  WORK_WIZ_PROJECT_07_TITLE:
    "자기주도학습 서비스 ‘위즈클래스’ 프로토타입 개발 ",
  WORK_WIZ_PROJECT_07_PERIOD: "2020.07",
  WORK_WIZ_PROJECT_07_DESC:
    "다른 회사와 합작할 서비스를 위해 프로토타입 페이지 제작",
  WORK_WIZ_PROJECT_07_OUTCOME: "위즈클래스 페이지",
  WORK_WIZ_PROJECT_07_ROLE:
    "프로젝트에 혼자 참여하여 프로토타입 전체 개발 완료",
  WORK_WIZ_PROJECT_07_DETAILS_01: "프로젝트 전체 구성",
  WORK_WIZ_PROJECT_07_DETAILS_02:
    "기존 api를 토대로 클라이언트만 새로 구성해 프로토타입 개발",

  WORK_WIZ_PROJECT_08_TITLE: "블록코딩 서비스 ‘타피오카’ 개발 및 최적화 ",
  WORK_WIZ_PROJECT_08_PERIOD: "2020.07 - 2020.10",
  WORK_WIZ_PROJECT_08_DESC:
    "블록코딩 및 교육용 단계별 코딩 수업 코스를 제공하는 모바일 앱",
  // WORK_WIZ_PROJECT_08_FEATURES:
  //   "모바일용 블록 코딩 에디터(웹뷰), 교육용 단계별 코딩 수업 코스",
  WORK_WIZ_PROJECT_08_OUTCOME: "중국 바이어 계약 완료, 국내 매출 발생",
  // WORK_WIZ_PROJECT_08_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_08_ROLE:
    "UI 개발 및 모바일 기능 추가 및 수업 코스 관련 서버 개발",
  WORK_WIZ_PROJECT_08_DETAILS_01:
    "게임 제작 관련 UI 개발 및 모바일용 새로운 기능 추가",
  WORK_WIZ_PROJECT_08_DETAILS_02:
    "교육용 단계별 코딩 수업 코스 관련 db 테이블 설계 및 api 개발",
  WORK_WIZ_PROJECT_08_DETAILS_03: "모바일에 최적화된 뷰 UI 개발",
  WORK_WIZ_PROJECT_08_DETAILS_04:
    "UI rendering 퍼포먼스 최적화 및 메모리 최적화",

  WORK_WIZ_PROJECT_09_TITLE: "서비스 ‘위즈랩’ 기능 업데이트",
  WORK_WIZ_PROJECT_09_PERIOD: "2020.10 - 2020.12",
  WORK_WIZ_PROJECT_09_DESC:
    "현재 서비스 되고있는 플랫폼에 여러 기능을 추가하여 새로운 버전 배포",
  // WORK_WIZ_PROJECT_09_FEATURES:
  //   "사용자 신고 기능 및 신고 당한 유저 서비스 범위 제한 / 사용자가 제작한 게임 apk 다운로드 기능 / 채팅 기능 업데이트",
  WORK_WIZ_PROJECT_09_OUTCOME: "서비스 내 유료 서비스 런칭 및 효율적 유저 관리",
  // WORK_WIZ_PROJECT_09_TECH: "react,js,node.js,mysql,HTML/CSS",
  WORK_WIZ_PROJECT_09_ROLE:
    "위즈랩 사용자 신고 기능 및 신고 당한 유저에 서비스 범위 제한 / 사용자가 제작한 게임 apk 다운로드 기능 업데이트 / 채팅 서비스 기능 업데이트",
  WORK_WIZ_PROJECT_09_DETAILS_01: "신고 관련 db 테이블 설계 및 api 개발",
  WORK_WIZ_PROJECT_09_DETAILS_02:
    "클라이언트에서 신고 당한 유저에게 특정 서비스 범위 제한",
  WORK_WIZ_PROJECT_09_DETAILS_03:
    "사용자의 번들링된 게임 파일을 Android의 웹뷰로 보여주어 apk로 제작",
  WORK_WIZ_PROJECT_09_DETAILS_04: "apk 과정 자동화를 위해 Zenkins 사용",
  WORK_WIZ_PROJECT_09_DETAILS_05:
    "apk 제작 서비스 관련 페이지 및 결제 api 개발",
  WORK_WIZ_PROJECT_09_DETAILS_06:
    "채팅 및 사용자 커뮤니티에 글/댓글/채팅 도배 금지,욕설 금지 기능 서버/클라이언트 개발",
  WORK_WIZ_PROJECT_09_DETAILS_07: "서버 성능 최적화를 위해 Redis 사용",

  WORK_WIZ_PROJECT_10_TITLE: "대교 온라인 강의 플랫폼 '방클' 서비스 개발",
  WORK_WIZ_PROJECT_10_PERIOD: "2021.01 - 2021.03",
  WORK_WIZ_PROJECT_10_DESC:
    "오프라인에서 실행되는 학교 방과 후 수업을 온라인으로 듣는 플랫폼",
  // WORK_WIZ_PROJECT_10_FEATURES:
  //   "수업예약, 수업 진도 확인, 과제 확인, 자동 출석 체크, 과제 제출, 수업 입장, 추천 튜터, 수업 신청 가능 조건 설정, 수강신청 등",
  WORK_WIZ_PROJECT_10_OUTCOME: "서비스 '방클' 런칭 ",
  WORK_WIZ_PROJECT_10_ROLE:
    "서버 리더로 참여하여 기획회의 참여 및 관련 서버 개발 ",
  // WORK_WIZ_PROJECT_10_TECH: "js,node.js,mysql",
  WORK_WIZ_PROJECT_10_DETAILS_01: "기획 회의 참여 후 개발 관련 피드백 지원",
  WORK_WIZ_PROJECT_10_DETAILS_02: "기획서 바탕으로 db 스키마 설계",
  WORK_WIZ_PROJECT_10_DETAILS_03:
    "수업예약, 수업 진도 확인, 과제 확인, 자동 출석 체크, 과제 제출, 수업 입장, 추천 튜터, 수업 신청 가능 조건 설정, 수강신청 등의 api 개발",
  WORK_WIZ_PROJECT_10_DETAILS_04:
    "실제 테스트 유저를 바탕으로 알파,베타 테스트 완료",

  /* hackers */
  WORK_HACKERS_PROJECT_01_TITLE: "웹용 전자 투표 개발 ",
  WORK_HACKERS_PROJECT_01_PERIOD: "2019.02 - 2019.04",
  WORK_HACKERS_PROJECT_01_DESC:
    "오프라인으로 진행되는 투표시스템을 웹으로 구현한 웹 사이트 개발",
  // WORK_HACKERS_PROJECT_01_FEATURES:
  //   "투표 프로젝트 생성/관리, 관리자 페이지, 사용자별 투표접근 제한 ",
  WORK_HACKERS_PROJECT_01_OUTCOME: "",
  // WORK_HACKERS_PROJECT_01_TECH: "PHP,jQuery,Bootstrap,JavaScript",
  WORK_HACKERS_PROJECT_01_ROLE:
    "프로젝트에 혼자 참여하여 클라이언트와 서버 개발",
  WORK_HACKERS_PROJECT_01_DETAILS_01:
    "관리자 대쉬보드 UI 개발 및 서버 api 개발",
  WORK_HACKERS_PROJECT_01_DETAILS_02: "투표 페이지 기능 개발 및 서버 api 개발",

  WORK_HACKERS_PROJECT_02_TITLE: "회사 코인 소개 웹 사이트 개발",
  WORK_HACKERS_PROJECT_02_PERIOD: "2019.04(1주일)",
  WORK_HACKERS_PROJECT_02_DESC:
    "회사 코인 홍보를 위해 Bootstrap을 사용하여 반응형 웹사이트 제작",
  WORK_HACKERS_PROJECT_02_FEATURES: "",
  WORK_HACKERS_PROJECT_02_OUTCOME: "",
  // WORK_HACKERS_PROJECT_02_TECH: "PHP,jQuery,Bootstrap,JavaScript,MariaDB",
  WORK_HACKERS_PROJECT_02_ROLE: "클라이언트 개발",
  WORK_HACKERS_PROJECT_02_DETAILS_01:
    "이메일 문의, 게시판관리, 이미지 업로드 등 UI 기능 구현",

  WORK_HACKERS_PROJECT_03_TITLE: "보안 솔루션 개발",
  WORK_HACKERS_PROJECT_03_PERIOD: "2019.06 - 2019.07",
  WORK_HACKERS_PROJECT_03_DESC:
    "pc 자원의 위험 추적 관리 , 위험 상황 실시간 알림 등 pc 보안 솔루션",
  // WORK_HACKERS_PROJECT_03_FEATURES:
  //   "pc 자원의 위험추적 관리 및 자산의 효과적 관리 기능, pc 자원의 보안 위험 상황을 실시, 위협정보이력관리, pc 자산 정보 이력 관리",
  WORK_HACKERS_PROJECT_03_OUTCOME: "",
  WORK_HACKERS_PROJECT_03_TECH: "PHP,jQuery,Bootstrap,JavaScript,MariaDB",
  WORK_HACKERS_PROJECT_03_ROLE:
    "웹 페이지와 DB 부분을 담당하여 관리자 페이지 개발",
  WORK_HACKERS_PROJECT_03_DETAILS_01:
    "로그 실시간 반영, 시간에 따른 그래프 로드, 파일 업로드 등 반응형 데쉬보드 구현",
  WORK_HACKERS_PROJECT_03_DETAILS_02:
    "웹소켓을 이용하여 실시간 로그 대쉬보드 UI 구현",
  // WORK_HACKERS_PROJECT_03_DETAILS_03: "crud api를 이용하여 DB 구현",

  WORK_HACKERS_PROJECT_04_TITLE: "청소 관련 웹앱 디버깅",
  WORK_HACKERS_PROJECT_04_PERIOD: "2019.07",
  WORK_HACKERS_PROJECT_04_DESC: "협력 업체의 청소 관련 웹/앱 디버깅 ",
  WORK_HACKERS_PROJECT_04_ROLE:
    "청소 협력 업체 웹/앱을 클라이언트 요구에 맞춰 수정 및 기능 지원",
  // WORK_HACKERS_PROJECT_04_FEATURES: "",
  WORK_HACKERS_PROJECT_04_OUTCOME: "",
  // WORK_HACKERS_PROJECT_04_TECH:
  //   "PHP,CI framework,Bootstrap,JavaScript,HTML/CSS",
  WORK_HACKERS_PROJECT_04_DETAILS_01:
    "PHP의 프레임워크인 CI로 개발된 프로젝트를 요구사항에 맞춰 수정",

  /* projects */
  PROJECT_DIVE_TITLE: "하루의 감정 기록하는 앱(dive) 개발",
  PROJECT_DIVE_AT: "사이드프로젝트",
  PROJECT_DIVE_PERIOD: "2020.08 - 2020.11",
  PROJECT_DIVE_OUTCOME: "구글 플레이 스토어, 앱스토어 출시",
  PROJECT_DIVE_ROLE: "아이디어 회의 참여 및 나의 점수기록 부분 ui 개발",
  PROJECT_DIVE_DESC:
    "하루 동안 내 기분이 어땠는지 기록하고 트래킹 할수있는 다이어리 앱 개발",
  PROJECT_DIVE_DETAILS_01: "flutter를 사용하여 ios, android 모두 지원",
  PROJECT_DIVE_DETAILS_02: "기획자,디자이너,개발자들과 팀을 꾸려 협업",

  PROJECT_LECTURE_TITLE: "수강신청 도우미 앱 개발",
  PROJECT_LECTURE_AT: "역량강화",
  PROJECT_LECTURE_PERIOD: "2018.11-2018.12",
  PROJECT_LECTURE_OUTCOME: "구글 플레이 스토어 출시",
  PROJECT_LECTURE_ROLE: "클라이언트 및 서버 개발 완료",
  PROJECT_LECTURE_DESC:
    "대학교 수강신청 기능을 모바일에서 가능하도록 지원하는 안드로이드 앱 개발 ",
  PROJECT_LECTURE_DETAILS_01: "수업 경쟁률 비교, 인기 강좌 순위",
  PROJECT_LECTURE_DETAILS_02: "본인 조건에 맞는 수강 가능 과목 확인",

  PROJECT_THEIF_TITLE: "키보드로 조작하는 2D 게임 개발",
  PROJECT_THEIF_AT: "학교 프로젝트",
  PROJECT_THEIF_PERIOD: "2016.03-2016.06",
  PROJECT_THEIF_ROLE: "각종 아이템 및 도둑 관련 파트 개발",
  PROJECT_THEIF_DESC: "c++ SDL 라이브러리를 활용하여 도둑잡기 고전 게임 개발 ",
  PROJECT_THEIF_DETAILS_01:
    "경찰(나)이 도둑을 잡는게임으로, 도둑이 뒤를 돌아보고 있는 동안 경찰이 움직이면 게임 종료",
  PROJECT_THEIF_DETAILS_02:
    "도둑이 앞을 보고 있을때 경찰(나)는 길에 있는 각종 아이템을 먹어서 점수를 올리는 방식",
  PROJECT_THEIF_DETAILS_03: "디자이너,개발자와 팀으로 협업",
};
export default ko;
